package com.caraxes.app.update
...
object AppUpdater {
    ...
    private const val APK_CACHE_NAME = "caraxes-update.apk"
    private const val APK_PATH = "/download/caraxes.apk"
    ...
    fun resolveDownloadUrl(baseUrl: String, raw: String): String {
        val path = when {
            raw.isBlank() -> APK_PATH
            raw.startsWith("/") -> raw
            else -> {
                val uri = Uri.parse(raw)
                if (isLoopbackHost(uri.host)) {
                    uri.path?.takeIf { it.isNotBlank() } ?: APK_PATH
                } else {
                    return raw
                }
            }
