#!/usr/bin/env bash
# 编译 Android release APK，复制到 server/downloads/caraxes.apk，并上传到服务器
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CLIENT="$ROOT/android"
OUT="$ROOT/server/downloads"
APK_NAME="caraxes.apk"
REMOTE_HOST="${CARAXES_UPLOAD_HOST:-root@S2}"
REMOTE_DIR="${CARAXES_UPLOAD_DIR:-/root/caraxes/server/downloads}"
REMOTE="${REMOTE_HOST}:${REMOTE_DIR}/${APK_NAME}"
CLEAN="${CARAXES_CLEAN:-1}"
...
if [[ "${CARAXES_UPLOAD:-1}" != "0" ]]; then
  echo "上传到 $REMOTE ..."
  if ssh "$REMOTE_HOST" "mkdir -p '$REMOTE_DIR'" \
    && scp "$OUT/$APK_NAME" "$OUT/app_version.json" "$REMOTE_HOST:$REMOTE_DIR/"; then
    echo "上传完成: $REMOTE"
  else
    echo "上传失败（本地 APK 已生成）。可稍后重试，或设置 CARAXES_UPLOAD=0 仅本地构建。" >&2
    exit 2
  fi
else
  echo "已跳过上传（CARAXES_UPLOAD=0）"
fi
